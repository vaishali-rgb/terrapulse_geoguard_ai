...
graph TD
    subgraph "Data Acquisition"
        S2["🛰️ Sentinel-2 L2A"] --> SH["Sentinel Hub API"]
        SH --> CM["☁️ Cloud Masking (SCL)"]
        CM --> SP["✨ Spectral Preprocessing"]
    end

    subgraph "Siamese-SNN Inference"
        SP --> SE["🧠 Siamese U-Net Encoder"]
        SE --> DM["📐 Difference Maps |F_A - F_B|"]
        DM --> RC["⚡ Rate Coding (Poisson)"]
        RC --> SD["🔬 SNN Decoder (LIF)"]
        SD --> SA["📊 Spike Accumulation (T=20)"]
    end

    subgraph "Classification & Compliance"
        SA --> SI["🌿 Spectral Index Analysis"]
        SI --> RE["⚖️ Compliance Rule Engine"]
        RE --> VS["🚨 Severity Scoring"]
    end

    subgraph "Audit & Delivery"
        VS --> BC["⛓️ Blockchain Evidence Hasher"]
        BC --> DB["💾 PostGIS & ChromaDB"]
        DB --> WS["🖥️ Real-time Dashboard & Chatbot"]
    end
...
- Integrate **14 specialized geospatial tools** the chatbot can invoke:
  ...
  - Compliance checks, violation listing#### Objective 5: Real-Time Performance
...
graph TB
    subgraph "Data Acquisition Layer"
        SH["🛰️ Satellite Hub API"]
        CM["☁️ Cloud Masking (SCL)"]
        SP["✨ Spectral Preprocessor"]
    end

    subgraph "AI Inference Engine (RTX 4050 Optimized)"
        subgraph "Hybrid Siamese-SNN"
    ...
    subgraph "Compliance & Evidence Layer"
        RE["⚖️ Rule Engine (GDCR 2017)"]
        SS["🚨 Severity Scorer"]
        BC["⛓️ Blockchain Audit (Merkle Tree)"]
    end

    subgraph "User Interface Layer"
        LF["🗺️ Leaflet.js Interactive Map"]
